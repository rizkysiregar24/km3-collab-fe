    ...
    <nav className="bg-white text-black shadow-md ">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <Link className="font-bold text-2xl hidden md:inline-flex items-center gap-2" to="/" title="Back to Home">
            <Logo size={36} />
            Terbang Tinggi
          </Link>
          ...
          <div className="hidden md:block">
            {isValidUser ? (
              <AuthRightElementNavbar handleLogout={handleLogout} isAdmin={isAdmin} username={userDataRedux.name} />
            ) : (
              <div className="ml-10 flex items-baseline space-x-4">
                <Link to="/login" className=" hover:bg-brand-darker-800 hover:text-white px-3 py-2 rounded-md text-sm font-medium border-brand border-2">
                  Login
                </Link>

                <Link to="/register" className="hover:bg-brand-darker-800  px-3 py-2 rounded-md text-sm font-medium bg-brand text-white border-brand border-solid border-2">
                  Register
                </Link>
              </div>
            )}
        ...
        <div className={isOpen ? "block" : "hidden"}>
          <div className="md:hidden" id="mobile-menu">
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
              <Link to="/login" className="hover:bg-gray-200 block px-3 py-2 rounded-md text-base font-medium">
                Login
              </Link>

              <Link to="/register" className=" hover:bg-gray-200 block px-3 py-2 rounded-md text-base font-medium">
                Register
              </Link>
            </div>
          </div>
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <div className="dropdown dropdown-end">
        <label tabIndex={0} className="btn btn-outline btn-primary btn-sm btn-circle avatar placeholder" title="Profile Menu">
